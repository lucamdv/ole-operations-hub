...
export function Header({ onOpenPalette }: { onOpenPalette: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-30 shrink-0 px-3 pt-[max(0.75rem,env(safe-area-inset-top))] sm:px-5">
      <div className="glass-bar flex min-h-14 items-center gap-2 rounded-2xl px-3 py-2 sm:gap-3">
        <MobileNav />

        <button
          onClick={onOpenPalette}
          className="group hidden h-9 min-w-0 max-w-md flex-1 items-center gap-2.5 rounded-full border border-border bg-surface-2/60 px-3 text-left transition duration-300 ease-[var(--ease-apple)] hover:border-primary/35 hover:bg-surface/80 sm:flex"
        >
        ...
        <button
          onClick={onOpenPalette}
          className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-border bg-surface/60 text-muted-foreground transition hover:bg-surface-2 sm:hidden"
        ...
        <div className="mr-1 hidden shrink-0 items-center gap-2 border-r border-border pr-3 lg:flex">
        ...
        <div className="relative">
          <button
            ...
            className="relative grid h-9 w-9 place-items-center rounded-full border border-border bg-surface/60 transition duration-300 ease-[var(--ease-apple)] hover:bg-surface-2"
          ...
          {openNotif && (
            <>
              ...
              <div className="glass-bar fixed left-2 right-2 top-[calc(4.75rem+env(safe-area-inset-top))] z-50 w-auto overflow-hidden rounded-2xl shadow-elevated animate-in fade-in slide-in-from-top-2 sm:absolute sm:left-auto sm:right-0 sm:top-11 sm:w-[380px]">
                          ...
                          <div className="flex-1 min-w-0 pr-6">
                            <div
                              className={cn(
                                ...
                                a.read ? "text-muted-foreground" : "text-foreground font-medium",
                              )}
                            >
                              {a.text}
                            </div>
        ...
        <div className="ml-1 flex items-center gap-2 border-l border-border pl-3">
          ...
          <div
            className="grid h-8 w-8 place-items-center rounded-full bg-gradient-primary text-[11px] font-semibold text-primary-foreground shadow-soft ring-1 ring-white/15"
            title={profile.nome}
          >
          ...
          <button
            ...
            className="grid h-9 w-9 place-items-center rounded-full border border-border bg-surface/60 text-muted-foreground transition duration-300 ease-[var(--ease-apple)] hover:border-destructive/40 hover:bg-destructive/10 hover:text-destructive"
            title="Sair"
            aria-label="Sair"
          >
